...
func CreateSocietyHandler(ctx context.Context, event events.APIGatewayProxyRequest, userCtx *models.UserContext) (events.APIGatewayProxyResponse, error) {
	var payload struct {
...
func main() {
	lambda.Start(middleware.WithCORS(middleware.WithAuth(CreateSocietyHandler)))
