...
func TestGetOrderHistory(t *testing.T) {
	...
	tests := []struct {
		...
		{"service error", &models.UserContext{ID: 1}, "?status=APPROVED",
		...
		{"success", &models.UserContext{ID: 2}, "?status=APPROVED",
			func(u *models.UserContext, s []order_status.Status) ([]*models.Order, error) {
...
func TestGetAllApprovedAwaitingOrders(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			initializer.OrderService = &FakeOrderService{GetApprovedAwaitingFn: func(userCtx *models.UserContext) ([]*models.Order, error) {
				return nil, nil
			}}
			h := handlers.NewOrderHandler(initializer.OrderService, log)
...
func TestGetLenderOrders(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			initializer.OrderService = &FakeOrderService{GetLenderOrdersFn: func(u *models.UserContext) ([]*models.Order, error) {
				return nil, nil
			}}
			h := handlers.NewOrderHandler(initializer.OrderService, log)
