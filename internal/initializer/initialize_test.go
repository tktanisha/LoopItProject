...
func mockInitDBRepos(t *testing.T, fn func(logger.LoggerInterface, db.DatabaseInterface) error) func() {
	original := initializer.InitDBRepos
	initializer.InitDBRepos = func(l logger.LoggerInterface, d db.DatabaseInterface) error {
		return fn(l, d)
	}
...
func TestInitServices_DBStorage(t *testing.T) {
	...
	restore := mockInitDBRepos(t, func(l logger.LoggerInterface, db db.DatabaseInterface) error {
		if db != mockDB {
