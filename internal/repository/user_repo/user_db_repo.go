...
func (r *UserDBRepo) FindAll() []models.User {
	...
	for rows.Next() {
		var u models.User
		if err := rows.Scan(&u.ID, &u.FullName, &u.Email, &u.PhoneNumber, &u.Address, &u.PasswordHash, &u.SocietyID, &u.Role, &u.CreatedAt); err != nil {
			continue
		}
...
func (r *UserDBRepo) FindByID(userID int) (*models.User, error) {
	...
	var u models.User
	if err := row.Scan(&u.ID, &u.FullName, &u.Email, &u.PhoneNumber, &u.Address, &u.PasswordHash, &u.SocietyID, &u.Role, &u.CreatedAt); err != nil {
...
func (r *UserDBRepo) Create(user *models.User) {
	...
	`
	err := r.db.QueryRow(query, user.FullName, user.Email, user.PhoneNumber, user.Address, user.PasswordHash, user.SocietyID, user.Role, time.Now()).Scan(&user.ID)
	if err != nil {
...
func (r *UserDBRepo) BecomeLender(userID int) error {
	...
	// // Update user role
	// _, err = tx.Exec("UPDATE users SET role=$1 WHERE id=$2", enums.RoleLender, userID)
	// if err != nil {
	// 	tx.Rollback()
	// 	return err
	// }
	...
	// err = r.lenderRepo.CreateWithTx(tx, &models.Lender{
	// 	ID:            userID,
	// 	IsVerified:    true,
	// 	TotalEarnings: 0.0,
	// })
