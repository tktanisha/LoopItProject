...
func (c *CategoryService) GetAllCategories() ([]models.Category, error) {
	...
	if err != nil {
		c.log.Error(fmt.Sprintf("Failed to fetch categories, error: %v", err))
		return nil, err
	}

	c.log.Info(fmt.Sprintf("Successfully fetched %d categories", len(categories)))
	return categories, nil
}

func (c *CategoryService) CreateCategory(name string, price, security float64) error {
	c.log.Info(fmt.Sprintf("Creating category: %s with price %.2f and security %.2f", name, price, security))
	...
	if err := c.categoryRepo.Create(category); err != nil {
		c.log.Error(fmt.Sprintf("Failed to create category: %s, error: %v", name, err))
		return err
	}

	c.log.Info(fmt.Sprintf("Category created successfully: %s", name))
	return nil
}
